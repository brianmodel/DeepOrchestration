...
if __name__ == "__main__":
    ...
    if not os.path.exists(OUT_DB):
        ...
        answer = input(
            "Aligned DB seems to be already existing. Tape yes to continue : "
        )
        if answer != "yes":
            quit()
        else:
    ...
    for pair in list_dir:
        ...
        if len(mid_files) != 2:
            raise Exception(pair_folder + " contains more than 2 midi files")

        # Read midi files
        prs = [Read_midi(e, quantization).read_file() for e in mid_files]

        # Align them
        pr0_aligned, _, pr1_aligned, _, _, _ = align_pianorolls(
            prs[0], prs[1], gapopen=3, gapextend=1
        )
